...
import { QuartetoLocalToday, DailyQuartetoEntry } from './types';
...
const defaultQuartetoLocalToday: QuartetoLocalToday = {
  id: '',
  letters: [],
  number: 0,
};

export function useQuartetoEngine(data: DailyQuartetoEntry) {
  ...
  const { localToday, updateLocalStorage } = useDailyLocalToday<QuartetoLocalToday>({
    key: SETTINGS.LOCAL_TODAY_KEY,
    gameId: data.id,
    challengeNumber: data.number ?? 0,
    defaultValue: defaultQuartetoLocalToday,
