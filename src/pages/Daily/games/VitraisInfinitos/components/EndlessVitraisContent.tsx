...
type InfiniteVitraisPuzzleProps = {
  data: InfiniteVitraisEntry;
  width: number;
  onSaveProgress?: (pieceOrder: number[]) => void;
};

export const InfiniteVitraisPuzzle: React.FC<InfiniteVitraisPuzzleProps> = ({
  data,
  onSaveProgress,
  width,
}) => {
  ...
  return (
    ...
    >
      ...
      {isSolved && (
        <div style={{ marginTop: '1rem', color: '#4ade80', fontWeight: 'bold' }}>
          <Translate
            pt="Parabéns! Você montou o vitral!"
            en="Congratulations! You completed the stained glass!"
          />
        </div>
      )}
