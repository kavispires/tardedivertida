import { CARD_TYPES } from 'games/escape-room/components/cards/escape-room-types';
import type {
  EscapeRoomCardType,
  EscapeRoomCompleteMissionCardType,
  EscapeRoomMissionCardType,
  EscapeRoomContentCardType,
} from 'games/escape-room/components/cards/escape-room-types';
// Ant Design Resources
import { Alert } from 'antd';
// Components
import { Translate } from 'components/language';
// Internal
import { Card, CenterBox, Content, ERSprite, Header, Subtitle, Title } from './CardBuildingBlocks';
...
const EscapeRoomCompleteMissionCard = ({
  ...
  return (
    <Card cardId={card.id} background={card.background} width={width} onPlayCard={onPlayCard}>
      <Content center>
        <CenterBox>
          <Title>
            <Translate pt="Completar Missão" en="Complete Mission" />
          </Title>
          <ERSprite id="complete-mission" width={10} />
          <Subtitle align="center">
            <Translate
              pt="Quando você achar que uma missão está completa, use esta carta."
              en="When you think a mission is complete, use this card."
            />
          </Subtitle>
        </CenterBox>
      </Content>
...
const EscapeRoomMissionCard = ({
  ...
  return (
    <Card cardId={card.id} background={card.background} width={width} onPlayCard={onPlayCard}>
      <Header spriteId="mission">
        <Translate pt="Missão" en="Mission" /> #{number}
      </Header>
      <Content>
        {content.map((entry, index) => (
          <ContentDelegator key={index} content={entry} width={width} />
        ))}
      </Content>
    </Card>
  );
