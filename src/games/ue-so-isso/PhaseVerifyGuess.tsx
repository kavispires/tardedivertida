...
export function PhaseVerifyGuess({ state, players }: PhaseProps) {
  ...
  const [controller, isUserTheController] = useWhichPlayerIsThe(
    "controllerId",
    state,
    players,
  );
  ...
  const isActionable =
    !["CONTINUE", "WIN"].includes(state.group.outcome) &&
    (isUserTheController || isHost);

  const announcement = (
    <PhaseAnnouncement
      icon={<AnimatedProcessingIcon />}
      title={
        <Translate pt="Processando o palpite..." en="Processing the guess..." />
      }
  ...
  return (
    <PhaseContainer
      phase={state?.phase}
      allowedPhase={PHASES.UE_SO_ISSO.VERIFY_GUESS}
    >
