...
export function StepGuessing({
  ...
  const prepareSubmitGuesses = useCallback(() => {
    const result = Object.entries(guesses).reduce(
      (acc: PlainObject, [clueKey, value]) => {
    ...
    const availableCells = shuffle(
      grid.filter(
        (cell) =>
          ...
          !usedCells.includes(cell.index),
      ),
    );
    const availableClues = clues.filter(
      (clue) => !usedClues.includes(getClueKey(clue)),
    );
    const newGuesses = availableClues.reduce(
      (acc: PlainObject, clueObj, index) => {
