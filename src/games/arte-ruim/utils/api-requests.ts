...
export function useOnSubmitVotingAPIRequest(setStep: UseStep['setStep']) {
  ...
  const request = useGameActionRequest({
    ...
    onError: () => setStep(0),
    successMessage: translate({
      pt: 'Avaliação enviada! Agora aguarde os outros jogadores',
      en: 'Evaluation sent successfully! Wait for the other players',
    }),
    errorMessage: translate({
      pt: 'Vixi, o aplicativo encontrou um erro ao tentar enviar sua avaliação',
      en: 'Oops, the application failed to send your evaluation',
    }),
  });
