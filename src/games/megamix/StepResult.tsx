...
export function StepResult({
  ...
  // Dynamic background
  useColorizeBackground(
    user,
    time.timeLeft > 13 ? round.current : round.current + 1,
  );
      ...
      <RuleInstruction type="event">
        ...
        {scoringType === "NORMAL" && (
          <Translate
            pt={
              <>
                Jogadores que votaram com a maioria entram ou continuam na Área
                VIP.
              </>
            }
            en={
              <>
                Players who voted with the majority join or stay in the VIP
                area.
              </>
            }
          />
        ...
        {scoringType === "TIE" && (
          <Translate
            pt={
              <>
                Empate entre 2 ou mais respostas. Quem já estava na Área VIP
                fica e ganha ponto.
              </>
            }
            en={
              <>
                It's a tie for 2 or more answers. Whoever was already in the VIP
                Area stays there and get points.
              </>
            }
          />
        )}
        {scoringType === "DRAW" && (
          <Translate
            pt={
              <>
                Cada um escolheu uma coisa diferente? Todo mundo fora da Área
                VIP!
              </>
            }
            en={
              <>Did everybody just choose something different? Everybody out!</>
            }
          />
        )}
      </RuleInstruction>
        ...
        {playersList.map((player) => {
          ...
          return (
            <div
              ...
              style={getPosition(
                isWinningArea ? winningCount : losingCount,
                isWinningArea ? 0 : 1,
                width,
              )}
            >
  ...
  (index: number, side: number, width: number) => {
    ...
    const getTopValue = (
      ...
      return (
        ...
        (ratio[remainderPosition] + multiplier) * increment
      );
    };
