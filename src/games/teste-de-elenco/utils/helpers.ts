...
export const chatGPTMoviePrompt = (
  movie: FeatureFilm,
  language: Language,
): string => {
  if (language === "en") {
...
export const getMovieSummary = (movie: FeatureFilm) => {
  ...
  const maleCount = roles.filter(
    (role) => role.candidates[role.actor!].gender === "male",
  ).length;
  const femaleCount = roles.filter(
    (role) => role.candidates[role.actor!].gender === "female",
  ).length;
  ...
  const uniqueAges = new Set(
    roles.map((role) => role.candidates[role.actor!].age),
  );
  const ageDiversity = (() => {
  ...
  const uniqueRaces = new Set(
    roles.map((role) => role.candidates[role.actor!].ethnicity),
  );
  ...
  if (
    ...
    const loveInterestGender: string =
      movie.roles.THE_LOVE_INTEREST.candidates[
        movie.roles.THE_LOVE_INTEREST.actor
      ].gender;

    const protagonistGender: string =
      movie.roles.THE_PROTAGONIST.candidates[movie.roles.THE_PROTAGONIST.actor]
        .gender;
